...
#   export SSH_DEPLOY_HOST="root@你的服务器公网IP"
#   export SSH_DEPLOY_PATH="/root/home_sticker_design"          # 服务器上仓库路径
#   export PM2_APP_NAME="decor-ai"                                 # 与 pm2 list 里 name 一致
#
# 可选：
#   export DEPLOY_BRANCH="main"    # 默认 main
#   export SKIP_PUSH=1             # 已推送过，只触发服务器构建重启
#   export SSH_BATCH_MODE=0        # 0=允许密码输入(默认)；1=仅密钥登录
#
# 执行：
...
ssh $SSH_OPTS "$SSH_DEPLOY_HOST" "cd '${SSH_DEPLOY_PATH}' && PM2_APP_NAME='${PM2_APP_NAME}' bash scripts/deploy-on-server.sh '${BRANCH}' '${PM2_APP_NAME}'"
